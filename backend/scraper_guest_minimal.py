...
class MinimalGuestScraper:
    def __init__(self, human_like=True):
        """
        ゲスト専用最小限スクレイパー（確実に動作する機能のみ）
...
async def main():
    # ゲスト専用最小限スクレイパー
    print("=== ゲスト専用最小限スクレイパー ===")
    scraper = MinimalGuestScraper(human_like=True)
    await scraper.setup()
    ...
    buzz_tweets = await scraper.get_buzz_tweets_from_users(
        usernames=main_users,
        count_per_user=30,
        top_n=10
    )
    
    print(f"\nバズツイート TOP {len(buzz_tweets)}")
    ...
    print(f"'Python' 検索結果: {len(keyword_tweets)}件")
    ...
    print(f"複数ユーザーから 'AI' 検索: {len(multi_search)}件")
    
    # データ保存
    if buzz_tweets:
        scraper.save_to_json(buzz_tweets, 'minimal_buzz_tweets.json')
